    ...
    def get_images(self, obj):
        ...
        for img in obj.product.images.all():
            if img.image and img.image.name:
                images_data.append({"src": img.image.url, "alt": obj.product.title})

        if not images_data:
            images_data.append({"src": "https://noimage/", "alt": obj.product.title})
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    )  # в модели нужно делать метод вычисления
