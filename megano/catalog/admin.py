...
from .models import Banner, Category, Product, ProductImage, Review, Sale, Specification, Tag
...
class ProductAdmin(admin.ModelAdmin):
    ...
    search_fields = ("id", "title", "category__title", "description", "full_description")
    prepopulated_fields = {"slug": ("title",)}
    list_editable = ("count",)
    list_per_page = 20
    date_hierarchy = "date"
    ...
    fieldsets = (
        ("Основная информация", {"fields": ("title", "slug", "category", "description", "full_description")}),
        (
            "Цена и наличие",
            {
        ...
        ("Статус товара", {"fields": ("is_active", "is_limited", "ordering_index")}),
        ("Статистика", {"fields": ("rating", "reviews_count", "purchase_count"), "classes": ("collapse",)}),
    )
    ...
    def current_price_display(self, obj):
        if obj.has_active_sale:
            return format_html(
                ...
                '{} $</span> <span style="color: gray; text-decoration: line-through;">{} $</span>',
                obj.current_price,
                obj.price,
            )
        return f"{obj.price} $"
...
class SaleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Товар и цена", {"fields": ("product", "sale_price")}),
        ("Период действия", {"fields": ("date_from", "date_to")}),
    )
