    ...
    def _check_stock(self, product, requested_count, current_count=0):
        ...
        if new_total > product.count:
            ...
            raise serializers.ValidationError(
                {
                    "id": f"Недостаточно товара на складе. "
                    f"Можно добавить не более {available} шт. (доступно всего: {product.count})"
                }
            )
        return True
    ...
    def save(self, **kwargs):
        ...
        try:
            ...
            if not basket_item.product.available:
                raise serializers.ValidationError({"id": f"Товар '{basket_item.product.title}' недоступен"})
        ...
        try:
            ...
            if not product.available:
                raise serializers.ValidationError({"id": f"Товар '{product.title}' недоступен"})
            ...
            raise serializers.ValidationError({"id": f"Товар с ID {item_id} не найден"})
    ...
    def delete(self, **kwargs):
        ...
        try:
            ...
            try:
                ...
                raise serializers.ValidationError({"id": f"Товар с ID {item_id} не найден в корзине"})
...
class BasketItemDetailSerializer(serializers.ModelSerializer):
    ...
    def get_date(self, obj):
        local_dt = timezone.localtime(obj.product.date)
    ...
    def get_images(self, obj):
        ...
        for img in product.images.all():
            if img.image and img.image.name:
                ...
                images_data.append(
                    {"src": image_url, "alt": img.alt if hasattr(img, "alt") and img.alt else product.title}
                )
        ...
        if not images_data:
            ...
            images_data.append({"src": default_url, "alt": product.title})
