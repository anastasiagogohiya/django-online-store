...
class Basket(models.Model):
    ...
    profile = models.OneToOneField(
        Profile, on_delete=models.CASCADE, null=True, blank=True, related_name="basket", verbose_name="Пользователь"
    )
    session_key = models.CharField(
        max_length=40, null=True, blank=True, db_index=True, verbose_name="Ключ сессии"
    ...
    def __str__(self):
        if self.profile:  # pragma: no cover
...
class BasketItem(models.Model):
    ...
    def __str__(self):
        return f"{self.product.title} x {self.count}"  # pragma: no cover
