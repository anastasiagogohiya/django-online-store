...
@extend_schema(
    ...
    request={"application/json": {"properties": {"username": {"type": "string"}, "password": {"type": "string"}}}},
    responses={
...
class SignInView(APIView):
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        В окошке авторизации вставьте Токен. Token ......"""
...
class SignUpView(APIView):
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        if not all([name, username, password]):
            return Response(
                {
                    "error": f"Поля name, username и password обязательны. "
                    f"Получено: name={name}, username={username}, password={'*' if password else None}"
                },
        ...
        if User.objects.filter(username=username).exists():
            return Response(
                {"error": "Пользователь с таким username уже существует"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"token": token.key, "user_id": user.id, "is_superuser": user.is_superuser}, status=status.HTTP_200_OK
        )


@extend_schema(
    summary="Выход из системы, удаление токена",
    request=None,
...
class SignOutView(APIView):
    ...
    def post(self, request):
        logger.info("Попытка выйти из системы...")
