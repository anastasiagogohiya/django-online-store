...
class ProfileView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        return Response(
            {"message": "Введите корректные данные.", "errors": serializer.errors}, status=status.HTTP_400_BAD_REQUEST
        )
...
@extend_schema(
    ...
    request={
        "multipart/form-data": {
            "type": "object",
            "properties": {
                "avatar": {"type": "string", "format": "binary", "description": "Файл изображения"},
                "alt": {"type": "string", "description": "Описание изображения"},
            },
            "required": ["avatar"],
        }
    },
    responses={
        200: {"type": "object", "properties": {"src": {"type": "string"}, "alt": {"type": "string"}}},
...
class ProfileAvatarUploadView(APIView):
    ...
    @catch_all_errors
    def post(self, request: Request) -> Response:
        logger.info("Пользователь пытается загрузить аватар...")
        ...
        # Создаем аватар
        avatar = Avatar.objects.create(src=file, alt=alt)
        ...
        return Response({"src": avatar.src.url, "alt": avatar.alt}, status=status.HTTP_200_OK)
